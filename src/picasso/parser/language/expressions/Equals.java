...
public class Equals extends AbstractOperator {
	...
	String operator = "" + CharConstants.EQUAL + CharConstants.EQUAL;

	/**
	 * Create an equals expression that takes as a parameter the given expressions
	 * 
	 * @param exp1 the left expression
	 * @param exp2 the right expression
	 */
	public Equals(ExpressionTreeNode exp1, ExpressionTreeNode exp2) {
		super(exp1, exp2);
	...
	public RGBColor evaluate(double x, double y) {
		...
		double error = 0.001;
		if ((Math.abs(result1.getBlue() - result1.getGreen()) > error)
				|| (Math.abs(result1.getGreen() - result1.getRed()) > error)) {
			throw new ParseException("Invalid Comparison. Comparison failed as compared values are not grayscale.");
		} else if ((Math.abs(result2.getBlue() - result2.getGreen()) > error)
				|| (Math.abs(result2.getGreen() - result2.getRed()) > error)) {
			throw new ParseException("Invalid Comparison. Comparison failed as compared values are not grayscale.");
		}
